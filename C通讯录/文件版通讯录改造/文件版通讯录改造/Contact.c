...
void SaveContact(struct Contact* ps)
{
	...
	//写通讯录中的数据到文件中
	fprintf(pfWrite, "%-20s\t%-4s\t%-5s\t%-12s\t%-20s\n", "名字", "年龄", "性别", "电话", "地址");
	for (i = 0; i < ps->size; i++)
	{
		fprintf(pfWrite, "%-20s\t%-4d\t%-5s\t%-12s\t%-20s\n",
			ps->data[i].name,
			ps->data[i].age,
			ps->data[i].sex,
			ps->data[i].address,
...
void LoadContact(struct Contact* ps)
{
	...
	fscanf(pfRead, "%*[^\n]\n");//skip the title line
	while (fscanf(pfRead, "%s %d %s %s %s", temp.name, &temp.age, temp.sex, temp.tele, temp.address) != EOF)
	{
		CheckCapacity(ps);
